...
TIM_OFF * timer_start_micros(char timer, int microS){
	int prescaler = 72-1;
	TIM_OFF * tim;
...
TIM_OFF * timer_start_millis(char timer, int millis){
...
void TDelay_Micros(char timer, int microS){
	...
	TIM_OFF * tim = timer_start_micros(timer, microS);
...
void timer_irq_millis_start(char timer, int milliS){
	TIM_OFF * tim = timer_start_millis(timer, milliS);
	tim->DIER |= 1;
...
TIM_OFF * get_timer_address(char port, char pin){
	if(port ==1)
	{
		if(pin < 4)
		{
...
void timer_compare_micro(char port, char pin, int arr, int compare){
	TIM_OFF * tim = get_timer_address(port, pin);
...
void tim_start_millis(char timer, int millis){
	...
	// Setup the prescaler to 72 -> 1 div equal to 1 Milli Sec
	tim->PSC = prescaler;	
...
void timer_capture_micro(char port, char pin, int arr, char edge){
	...
	if ((tim->CCER & (1 << ((channel-1)*4)) ) == 0)
	{
		...
		//Capture/Compare selection: configure channel as input
		switch(channel)
		{
...
void timer_capture_milli(char port, char pin, int arr, char edge){
	...
	if ((tim->CCER & (1 << ((channel-1)*4)) ) == 0)
	{
		...
		switch(timer)
		{
			case 1: NVIC_EnableIRQ(TIM1_CC_IRQn);	break;
			case 2: NVIC_EnableIRQ(TIM2_IRQn);    break;
			case 3: NVIC_EnableIRQ(TIM3_IRQn);    break;
			case 4: NVIC_EnableIRQ(TIM4_IRQn);    break;
		}
